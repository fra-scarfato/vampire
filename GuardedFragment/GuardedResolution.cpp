...
void GuardedResolution::attach(SaturationAlgorithm* salg)
{
  ASS(!_index);
  ...
  _unificationWithAbstraction = env.options->unificationWithAbstraction()!=Shell::Options::UnificationWithAbstraction::OFF;
}

void GuardedResolution::detach()
{
  ASS(_salg);
...
struct GuardedResolution::UnificationsFn
{
...
struct GuardedResolution::ResultFn
{
  ResultFn(Clause* cl, PassiveClauseContainer* passiveClauseContainer, bool afterCheck, Ordering* ord, LiteralSelector& selector, GuardedResolution& parent)
  ...
  Clause* operator()(pair<Literal*, SLQueryResult> arg)
  {
    ...
    return GuardedResolution::generateClause(_cl, resLit, qr, _parent.getOptions(), _passiveClauseContainer, _afterCheck ? _ord : 0, &_selector);
  }
private:
  Clause* _cl;
  ...
  GuardedResolution& _parent;
};

bool checkMaximal(Literal* maximalLit, Clause* cl)
{
    auto literals = cl->getLiteralIterator();
    int max = maximalLit->getVarDepth();
    
    while (literals.hasNext()){
        auto lit = literals.next();

        /* maximal literal should have max vardepth between clause literals*/
        if (max < lit->getVarDepth()) {
            if (!maximalLit->containsAllVariablesOf(lit)) return false; // check effectivly if maxLit contains every variable of lit
        }
    }
    return true;
}
 ...
 */
Clause* GuardedResolution::generateClause(Clause* queryCl, Literal* queryLit, SLQueryResult qr, const Shell::Options& opts, PassiveClauseContainer* passiveClauseContainer, Ordering* ord, LiteralSelector* ls)
{
    ASS(qr.clause->store()==Clause::ACTIVE);//Added to check that generation only uses active clauses
    
    DHSet<unsigned> queryVars;
    DHSet<unsigned> queryResultVars;
    queryCl->collectVars(queryVars);
    qr.clause->collectVars(queryResultVars);
    auto it = queryVars.iterator();

    /* 1. clauses must have no variables in common*/
    // while (it.hasNext()) {
    //     if (queryResultVars.contains(it.next())) return 0;
    // }

    /*2. check if query literal and query result literal are maximal
    in their clause*/
    if (!checkMaximal(queryLit, queryCl) || !checkMaximal(qr.literal, qr.clause)) return 0;
    ...
    */
    ...
    for(unsigned i=0;i<constraints->size();i++){
        ...
        static Shell::Options::UnificationWithAbstraction uwa = opts.unificationWithAbstraction();
        if(uwa==Shell::Options::UnificationWithAbstraction::GROUND &&
            !constraint->ground() &&
    ...
    GuardedPreprocess prepro = GuardedPreprocess(opts);
    prepro.computeVarDepth(res);
    //cout << "RESULT " << res->toString() << endl;
    return res;
}

ClauseIterator GuardedResolution::generateClauses(Clause* premise)
{
